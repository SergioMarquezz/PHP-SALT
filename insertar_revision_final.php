<?php
    ...
    $kilometros_entrada = $_POST['kilometros_entrada'];
    $total_km_recorridos = $_POST['total_km_recorridos'];
    ...
    $nombre_frontal = $id_viaje.'-frontal-llegada'.".".$extension_frontal;                    
    $nombre_trasera = $id_viaje.'-trasera-llegada'.".".$extension_trasera;                    
    $nombre_izquierda = $id_viaje.'-izquierda-llegada'.".".$extension_izquierda;                    
    $nombre_derecha = $id_viaje.'-derecha-llegada'.".".$extension_derecha;                    

    //Declaración ruta a guardar
    $ruta_frontal = "../img/Revision_Llegada/Frontal/".$nombre_frontal;
    $ruta_trasera = "../img/Revision_Llegada/Trasera/".$nombre_trasera;
    $ruta_izquierda = "../img/Revision_Llegada/Izquierda/".$nombre_izquierda;
    $ruta_derecha = "../img/Revision_Llegada/Derecha/".$nombre_derecha;
    ...
    if ($opcion == 'registrar') {
        ...
        try{
            ...
            }else if ($size_izquierda > 4194304){
            ...
            if ($foto_frontal && $foto_trasera && $foto_izquierda && $foto_derecha) {
                ...
                $consulta = ("CALL insert_revision_final(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);");
                $resultado = $conexion->prepare($consulta);             
                $resultado->execute(array($fecha_llegada, $kilometros_entrada, $total_km_recorridos, $aceite_motor, $aceite_transmision, $anticongelante, $liquido_frenos,
